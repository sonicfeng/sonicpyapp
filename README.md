...
4. Read the file [CONTRIBUTING.mdma](CONTRIBUTING.md)
...
- 🎯 Entry points to execute your program using `python -m <sonicpyapp>` or `$ sonicpyapp` with basic CLI argument parsing.
...
---
# sonicpyapp Flask Application

Awesome sonicpyapp created by sonicfeng
...
```bash
git clone https://github.com/sonicfeng/sonicpyapp sonicpyapp
cd sonicpyapp
make install
```
...
```bash
sonicpyapp create-db   # run once
sonicpyapp populate-db  # run once (optional)
sonicpyapp add-user -u admin -p 1234  # ads a user
sonicpyapp run
```

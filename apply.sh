...
original_author="sonicfeng"
original_name="sonicpyapp"
original_urlname="sonicpyapp"
original_description="Awesome sonicpyapp created by sonicfeng" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/sonicpyapp "${name}"
...
then
    ...
    echo "then run 'sonicpyapp' or 'python -m sonicpyapp'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'sonicpyapp' or 'python -m sonicpyapp' or 'poetry run sonicpyapp'"
fi
